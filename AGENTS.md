...
│     │     ├─ interface_packages.py
│     │     ├─ interface_apply.py
│     │     ├─ interface_receive_runtime.py
│     │     ├─ interface_value_converter.py
│     │     ├─ topic/
│     │     ├─ service/
│     │     ├─ action/
│     │     └─ node/
...
= Service graph 조회 / filter / status / alert / allowlist active_check 로직
  명시적 Service Call은 service/call_runtime.py에서 별도로 담당한다.

action/
= Action graph 조회 / status-feedback topic 관찰 / result 관찰 / alert 로직
  명시적 Action Goal은 action/goal_runtime.py에서 별도로 담당한다.
...
주요 endpoint는 `backend/src/ros2_dashboard_backend/ros2_dashboard_backend/main.py`에서 정의한다.
...
사용자가 명시적으로 수신 시작/중지를 누른 Topic만 `InterfaceReceiveRuntime`이 구독하고 history를 관리한다.
...
interface_value_converter.py가 scalar, sequence, nested custom msg, custom msg array를 재귀 변환한다.
...
goal 변환은 interface_value_converter.py의 recursive converter 원칙을 따른다.
validation 실패 시 sent_to_server=false로 기록하고 ROS server에 보내지 않는다.
...
```text
manual_interfaces.py의 scan_uploaded_interface_files()로 실제 남은 .msg/.srv/.action 파일을 다시 스캔한다.
manual_interfaces.py의 regenerate_uploaded_interfaces_cmake()는 append하지 않고 CMakeLists.txt 전체를 다시 쓴다.
manual_interfaces.py의 regenerate_uploaded_interfaces_package_xml()도 현재 파일 수 기준으로 전체를 다시 쓴다.
manual_interfaces.py의 regenerate_uploaded_interfaces_package()는 위 과정을 묶는 재사용 함수다.
```
...
```text
InterfaceReceiveRuntime만 명시적 Topic subscription을 생성한다.
...
ServiceCallRuntime의 명시적 호출 history에서 response history를 제공한다.
...
ActionGoalRuntime의 사용자 Goal 실행에서 발생한 feedback/result event를 history로 제공한다.

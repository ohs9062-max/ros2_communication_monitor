...
등록 interface 기반 명시적 Service call 구현 범위:

```text
Service call은 사용자가 UI에서 실행 버튼을 누른 경우에만 수행한다.
호출 후보는 interface_registry.yaml에 등록된 .srv 중 import_available=true이고,
현재 ROS2 graph에 같은 service_type으로 server_count >= 1인 Service만 포함한다.
GET /ros/interfaces/callable-services는 위 조건을 만족하는 Service와
parsed.request 기반 request schema를 반환한다.
POST /ros/interfaces/service-call은 service_name, service_type, request,
timeout_sec를 받아 위 조건을 다시 검증한 뒤 rclpy client.call_async로 호출한다.
response는 JSON-safe dict로 변환하고 최근 실행 history를 저장한다.
기존 ServiceRuntime active_check와 섞지 않고 별도 explicit call runtime으로 관리한다.
Action Goal 전송과 Topic publish는 구현하지 않는다.
Graph에 보이는 모든 Service를 자동 호출하거나 숨은 Service를 임의 호출하지 않는다.
```

등록 interface 기반 명시적 Action Goal 구현 범위:

```text
Action Goal은 사용자가 UI에서 실행 버튼을 누른 경우에만 전송한다.
실행 후보는 interface_registry.yaml에 등록된 .action 중 import_available=true이고,
현재 ROS2 graph에 같은 action_type으로 server_count >= 1인 Action만 포함한다.
GET /ros/interfaces/callable-actions는 위 조건과 parsed.goal/result/feedback schema를 반환한다.
POST /ros/interfaces/action-goal은 action_name, action_type, goal, timeout_sec를 받아
위 조건을 다시 검증한 뒤 rclpy ActionClient.send_goal_async로 전송한다.
accepted/rejected, feedback, result를 JSON-safe dict로 변환하고 최근 실행 history를 저장한다.
기존 ActionRuntime은 graph/status/feedback/result 관찰용으로 유지하고,
명시적 Goal 전송은 별도 ActionGoalRuntime에서 관리한다.
Topic publish는 구현하지 않는다.
...
Interface upload/apply 구현 범위:

```text
Overview 타입 업로드는 .msg/.srv/.action 파일을 registry에 저장하고
backend/src/ros2_dashboard_interfaces 패키지의 msg/srv/action 파일,
CMakeLists.txt, package.xml에 반영한다.
업로드 직후 registry build.rebuild_required는 true로 표시될 수 있다.
적용하기는 backend workspace 경로에서 colcon build --symlink-install을
subprocess로 실행한다.
build stdout/stderr는 backend/config/interface_apply_last.log에 저장하고,
마지막 build 상태는 backend/config/interface_apply_status.yaml에 저장한다.
동시에 apply가 여러 번 실행되지 않도록 lock으로 막는다.
build 성공 시 응답을 먼저 반환한 뒤 backend package 내부 reload_trigger.py를
갱신해 uvicorn --reload 감지를 유도한다.
build 실패 시 reload_trigger.py를 갱신하지 않는다.
재연결 후 POST /ros/interfaces/import-check로 generated Python import 가능 여부를
registry에 다시 반영한다.
backend 프로세스를 직접 kill/restart하거나 systemd/tmux를 제어하지 않는다.
Service request, Action Goal/cancel, Topic publish 기능과 연결하지 않는다.
```
...
2026-07-15 Interface upload 적용하기 및 reload 유도 구현:

```text
POST /ros/interfaces/apply는 backend workspace에서
source /opt/ros/jazzy/setup.bash 후 colcon build --symlink-install을 실행한다.
build 결과는 backend/config/interface_apply_status.yaml과
backend/config/interface_apply_last.log에 저장해 reload 후에도 조회 가능하다.
동시 apply는 lock으로 막고, build 실패 시 reload trigger를 갱신하지 않는다.
build 성공 후 backend/install/*/lib/python*/site-packages 경로를 현재 backend
프로세스의 sys.path 앞쪽에 반영하고 importlib cache를 갱신한 뒤 import-check를
즉시 재실행한다.
최종 성공은 file_saved, cmake_registered, package_xml_checked, colcon build,
install Python path 반영, import_available이 모두 성공한 경우에만 real_apply_success=true다.
build 성공이지만 import-check 실패면 status=import_failed로 저장한다.
최종 성공 시 응답 반환 후 BackgroundTask가
backend/src/ros2_dashboard_backend/ros2_dashboard_backend/reload_trigger.py를
갱신해 uvicorn --reload를 유도한다.
GET /ros/interfaces/apply/status는 마지막 build 상태와 log tail을 반환한다.
POST /ros/interfaces/import-check는 registry의 import_available/import_error를
generated interface Python import 결과로 갱신한다.
Frontend Overview 타입 업로드 영역은 적용하기 버튼, build 중 비활성화,
reload 대기/재연결 상태, 실패 log tail, 재연결 후 자동 import-check를 제공한다.
Service request, Action Goal/cancel, Topic publish, backend kill/restart는 구현하지 않는다.
```

...
const ALERT_SORT_COLUMNS = {
  ...
  detected_at: {
    ...
    value: (alert) => alert.detected_at,
  },
}

export function AlertsList({ alerts, onAlertClick }) {
  ...
  if (!alerts.length) {
    return <div className="empty-state">현재 Alert가 없습니다</div>
  }
      ...
      <table className="topic-table">
        <thead>
          <tr>
            ...
            <SortableHeader columnKey="detected_at" label="시간" onSort={onSort} sort={sort} />
          </tr>
        </thead>
        <tbody>
          {sortedAlerts.map((alert) => (
            ...
            >
              ...
              <td>{alert.message}</td>
              <td>{alert.code}</td>
              <td>{formatTime(alert.detected_at)}</td>
            </tr>
          ))}
        </tbody>
      </table>

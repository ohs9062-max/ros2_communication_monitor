...
이 프로젝트는 복잡한 ROS2 시스템에서 현재 어떤 데이터가 흐르고 있고,
어떤 서비스와 액션이 작동 중인지 한눈에 보기 위한 모니터링 대시보드입니다.
...
## API & WebSocket 전략
- **REST API (Polling)**: 페이지 진입이나 수동 갱신 때 사용하며,
  상세 상태 정보를 가져옵니다.
- **WebSocket (Push)**: 백엔드가 주기적으로 가벼운 상태 snapshot을 보내며,
  실시간 연결 상태와 요약을 보조합니다.
